...
# Function to detect and skip the entire marker sequence (start or end)
...
def decode_audio_in_real_time():
    ...
    while True:
        ...
        if not start_marker_detected:
            ...
            skip_marker(stream, START_MARKER_FREQS)
...
def save_decoded_data(decoded_data, filename):
...
if __name__ == "__main__":
    ...
    if action == "1":
        ...
        # Choose decoding mode (real-time or file-based)
        mode = input("Choose decoding mode (1 = Real-time, 2 = File-based): ").strip()
        
        if mode == "1":
            decoded_data = decode_audio_in_real_time()
            decoded_filename = input("Enter the filename to save decoded data: ")
            save_decoded_data(decoded_data, decoded_filename)

        elif mode == "2":
            input_audio_filename = input("Enter the filename to decode: ")
            decoded_data = decode_audio_from_file(input_audio_filename)
            decoded_filename = input("Enter the filename to save decoded data: ")
            save_decoded_data(decoded_data, decoded_filename)
